...
IF %AUDIO.VALUE%==TRUE (
	IF %AUDIO.VALUE%==TRUE IF %VOLUME% NEQ 0 START "" /MIN "%AUDIOMANAGER%" STOPALL
	SET NEW.VALUE=FALSE
) ELSE ( 
	SET NEW.VALUE=TRUE
)
...
CALL "%SETTINGS.LOAD%"
IF %AUDIO.VALUE%==TRUE IF %VOLUME% NEQ 0 START "" /MIN "%AUDIOMANAGER%" START system\villageambiance.mp3 menu
EXIT /B 0
