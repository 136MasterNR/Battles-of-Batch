...
IF NOT DEFINED QMEM_LOSE_SEEN IF NOT [%QMEM_LOSE%]==[%Q.LOSE%] IF NOT %Q.LOSE% GEQ %QMAX.LOSE% (SET QMEM_LOSE=%Q.LOSE%) ELSE SET QMEM_LOSE_SEEN=TRUE&SET "QMEM_LOSE=%Q.LOSE%"&CALL "%SAVE%" 1 PLAYER.MONEY %PLAYER.MONEY% +%QREW.MONEY.LOSE%&CALL "%SAVE%" 2 PLAYER.XP %PLAYER.XP% +%QREW.XP.LOSE%&CALL "%SCRIPTS_POP%\quest.cmd" %Q.LOSE% %QMAX.LOSE% 3 %QREW.MONEY.LOSE% %QREW.XP.LOSE% %QNAME.LOSE%
GOTO :EOF
:SKIP-1
