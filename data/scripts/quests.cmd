...
IF NOT DEFINED QMEM_TOTAL_MONSTERS_SEEN IF NOT [%QMEM_TOTAL_MONSTERS%]==[%Q.TOTAL_MONSTERS%] IF NOT %Q.TOTAL_MONSTERS% GEQ %QMAX.TOTAL_MONSTERS% (SET QMEM_TOTAL_MONSTERS=%Q.TOTAL_MONSTERS%) ELSE SET QMEM_TOTAL_MONSTERS_SEEN=TRUE&SET "QMEM_TOTAL_MONSTERS=%Q.TOTAL_MONSTERS%"&CALL "%SAVE%" "FILE=%DATA_SAVES%\PLAYERDATA.cmd" 1 PLAYER.MONEY %PLAYER.MONEY% +%QREW.MONEY.TOTAL_MONSTERS%&CALL "%SAVE%" "FILE=%DATA_SAVES%\PLAYERDATA.cmd" 2 PLAYER.XP %PLAYER.XP% +%QREW.XP.TOTAL_MONSTERS%&CALL "%SCRIPTS_POP%\quest.cmd" %Q.TOTAL_MONSTERS% %QMAX.TOTAL_MONSTERS% 5 %QREW.MONEY.TOTAL_MONSTERS% %QREW.XP.TOTAL_MONSTERS% %QNAME.TOTAL_MONSTERS%

IF NOT DEFINED QMEM_LOSE IF %Q.LOSE% GEQ %QMAX.LOSE% ( SET QMEM_LOSE_SEEN=TRUE ) ELSE SET "QMEM_LOSE=%Q.LOSE%"
IF NOT DEFINED QMEM_LOSE_SEEN IF NOT [%QMEM_LOSE%]==[%Q.LOSE%] IF NOT %Q.LOSE% GEQ %QMAX.LOSE% (SET QMEM_LOSE=%Q.LOSE%) ELSE SET QMEM_LOSE_SEEN=TRUE&SET "QMEM_LOSE=%Q.LOSE%"&CALL "%SAVE%" "FILE=%DATA_SAVES%\PLAYERDATA.cmd" 1 PLAYER.MONEY %PLAYER.MONEY% +%QREW.MONEY.LOSE%&CALL "%SAVE%" "FILE=%DATA_SAVES%\PLAYERDATA.cmd" 2 PLAYER.XP %PLAYER.XP% +%QREW.XP.LOSE%&CALL "%SCRIPTS_POP%\quest.cmd" %Q.LOSE% %QMAX.LOSE% 4 %QREW.MONEY.LOSE% %QREW.XP.LOSE% %QNAME.LOSE%

IF NOT DEFINED QMEM_TLVLS IF %COMPLETED.MAPS% GEQ %QMAX.TLVLS% ( SET QMEM_TLVLS_SEEN=TRUE ) ELSE SET "QMEM_TLVLS=%COMPLETED.MAPS%"
IF NOT DEFINED QMEM_TLVLS_SEEN IF NOT [%QMEM_TLVLS%]==[%COMPLETED.MAPS%] IF NOT %COMPLETED.MAPS% GEQ %QMAX.TLVLS% (SET QMEM_TLVLS=%Q.LOSE%) ELSE SET QMEM_TLVLS_SEEN=TRUE&SET "QMEM_TLVLS=%COMPLETED.MAPS%"&CALL "%SAVE%" "FILE=%DATA_SAVES%\PLAYERDATA.cmd" 1 PLAYER.MONEY %PLAYER.MONEY% +%QREW.MONEY.TLVLS%&CALL "%SAVE%" "FILE=%DATA_SAVES%\PLAYERDATA.cmd" 2 PLAYER.XP %PLAYER.XP% +%QREW.XP.TLVLS%&CALL "%SCRIPTS_POP%\quest.cmd" %COMPLETED.MAPS% %QMAX.TLVLS% 7 %QREW.MONEY.TLVLS% %QREW.XP.TLVLS% %QNAME.TLVLS%
EXIT /B 0
:INTERFACE
