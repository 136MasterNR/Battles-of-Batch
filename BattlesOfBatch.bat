...
::     UNLESS YOU ARE A TESTER AND THE OFFICIAL HTSSOFT-STUDIOS DEVELOPERS TOLD YOU TO DO SO (!)                                       ::
::                                                                                                                                     ::
:: - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ::
::                                                                                                                                     ::
:LAUNCHER
...
IF /I %CHOICE.INPUT%.==Z. SET /P "=[4C%RGB.FALSE%UI_ERR: CHAR_CUST UNAVAILABLE   [2G"<NUL
IF /I %CHOICE.INPUT%.==X. SET /P "=[4C%RGB.FALSE%UI_ERR: CHAR_HIST UNAVAILABLE   [2G"<NUL
...
IF "%SHOP.TAB%"=="1" (
	IF /I "%UDERFINE:~0,4%"=="HEAL" (
		CALL "%ITEMS.LOADER%" SHOP-ADD Heal 1 250 %SHOP.PRICE.HEAL% %ITEM.REG_LVL.Heal% %UDERFINE:~5%
	)
	IF /I "%UDERFINE:~0,8%"=="BUY HEAL" (
		CALL "%ITEMS.LOADER%" SHOP-ADD Heal 1 250 %SHOP.PRICE.HEAL% %ITEM.REG_LVL.Heal% %UDERFINE:~9%
	)
	IF /I "%UDERFINE:~0,4%"=="BOMB" (
		CALL "%ITEMS.LOADER%" SHOP-ADD Bomb 1 250 %SHOP.PRICE.BOMB% %ITEM.REG_LVL.Bomb% %UDERFINE:~5%
	)
	IF /I "%UDERFINE:~0,8%"=="BUY BOMB" (
		CALL "%ITEMS.LOADER%" SHOP-ADD Bomb 1 250 %SHOP.PRICE.BOMB% %ITEM.REG_LVL.Bomb% %UDERFINE:~9%
	)
) ELSE (
